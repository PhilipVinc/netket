...
def _mat_vec(jvp_fn, vjp_fn, v, diag_shift, pdf=None):
    (w,) = tree_conj(vjp_fn(v.conj()))
    if pdf is None:
        w = jax.tree_map(
            lambda x: subtract_mean(x * (1.0 / (v.size * mpi.n_nodes))), w
        )  # w/ MPI
    else:
        w = jax.tree_map(lambda x: pdf * (x - mpi.mpi_sum_jax(pdf @ x)[0]), w)  # w/ MPI
...
def _mat_vec_chunked(forward_fn, params, samples, v, diag_shift, pdf=None):
    w = tree_conj(_O_vjp(forward_fn, params, samples, v.conj()))
    if pdf is None:
        ...
        w = chunk_fn(w_)
    res = _O_jvp(forward_fn, params, samples, w)
    res = jax.tree_map(lambda x: mpi.mpi_sum_jax(x)[0], res)  # MPI
    return diag_shift * v + res
