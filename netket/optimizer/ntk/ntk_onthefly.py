...
from .ntk_onthefly_logic import mat_vec_factory, mat_vec_chunked_factory

from ..linear_operator import LinearOperator, Uninitialized


def NTKOnTheFly(vstate=None, *, chunk_size=None, **kwargs) -> "NTKOnTheFlyT":
    """
    ...
    """
    if vstate is None:
        return partial(NTKOnTheFly, chunk_size=chunk_size, **kwargs)

    if kwargs.pop("diag_scale", None) is not None:
        raise NotImplementedError(
            "\n`diag_scale` argument is not yet supported by NTKOnTheFly."
            "Please use `NTKJacobianPyTree` or `NTKJacobianDense`.\n\n"
...
@struct.dataclass
class NTKOnTheFlyT(LinearOperator):
    """
    Lazy representation of an S Matrix computed by performing 2 jvp
    and 1 vjp products, using the variational state's model, the
    samples that have already been computed, and the vector.
    ...
    _mat_vec: Callable[[Array, float], Array] = Uninitialized
    ...
    def _solve(self, solve_fun, y: Array, *, x0: Optional[Array], **kwargs) -> PyTree:
    ...
    def __repr__(self):
        return f"NTKOnTheFly(diag_shift={self.diag_shift})"


@jax.jit
def onthefly_mat_treevec(S: NTKOnTheFly, vec: Array) -> Array:
    """
...
@jax.jit
def _solve(
    self: NTKOnTheFlyT, solve_fun, y: Array, *, x0: Optional[Array], **kwargs
) -> Array:
